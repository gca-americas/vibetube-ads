...
export default function Simulator({ 
  ...
  const fetchState = async () => {
    ...
    if (attempt === 3) {
      try {
        const scriptRes = await fetch('/campaign/script?file=agent_bidding_policy.py');
        if (scriptRes.ok) {
          const scriptData = await scriptRes.json();
          if (scriptData.exists === false || !scriptData.script || scriptData.script.trim().length === 0) {
            setPolicyNotGenerated(true);
          } else {
            setPolicyNotGenerated(false);
          }
        }
      } catch (e) {
        setPolicyNotGenerated(true);
  ...
  const runFullSimulation = async () => {
    ...
    // Check if policy has not been generated yet
