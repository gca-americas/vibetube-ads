  ...
  return (
    <div className="animate-rise pb-24 space-y-8 max-w-6xl mx-auto">
      ...
      <div className="p-8 bg-card rounded-3xl border border-hairline shadow-2xl relative overflow-hidden space-y-6">
        ...
        {/* LEVEL 1: FORWARD PIPELINE (The Actor -> Candidate Policy -> The Critic with Actuator Tool) */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-stretch py-2 relative">
          {/* Node 1: Actor (Generator Agent) - Cols 1 to 5 */}
          <div 
            onClick={() => setActiveTab('wiring')}
            className={`lg:col-span-5 p-5 rounded-3xl border-2 transition-all flex flex-col justify-between space-y-3 relative cursor-pointer group ${
              activeNode === 1
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2.5 min-w-0">
                  <div className="w-9 h-9 rounded-xl bg-vibe-cyan/15 border border-vibe-cyan/30 flex items-center justify-center text-vibe-cyan shrink-0">
                    <Bot size={19} />
                  </div>
                  <div className="min-w-0">
                    <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-vibe-cyan block truncate">
                      1. The Actor
                    </span>
                    <h4 className="text-sm font-bold font-mono text-fg truncate">
                      Generator Agent
                    </h4>
                  </div>
                </div>
                <span className="shrink-0 text-[10px] font-mono px-2.5 py-0.5 rounded-full bg-overlay border border-hairline text-fg-muted whitespace-nowrap">
                  agent.py
                </span>
              </div>

              <p className="text-xs text-fg-muted font-sans leading-relaxed">
                Gemini 2.5 Flash authors the candidate Python bidding policy (<code className="text-fg font-mono text-[11px]">compute_bid</code>) guided by campaign constraints and BigQuery market clearing telemetry.
              </p>
            </div>

            <div className="pt-1">
              <div className="text-[10px] font-mono py-1.5 px-3 rounded-xl bg-overlay border border-hairline flex items-center justify-between">
                <span className="text-fg-muted">Agent State:</span>
                <span className="font-bold text-fg">
                  {activeNode === 1 ? '⚡ Synthesizing Candidate Code...' : flywheelCompleted ? '✓ Champion Policy Synthesized' : 'Awaiting Generation'}
                </span>
              </div>
            </div>
          </div>

          {/* Forward Code Transmission Channel - Cols 6 to 7 */}
          <div className="lg:col-span-2 hidden lg:flex flex-col items-center justify-center py-4 px-1 text-fg-muted">
            <span className="text-[9px] font-mono font-bold uppercase tracking-wider text-center text-vibe-cyan/90 mb-1">
              Candidate Policy
            </span>
            <div className="w-full flex items-center justify-center my-0.5">
              <div className={`h-0.5 w-full transition-colors ${activeNode === 1 || activeNode === 2 ? 'bg-vibe-cyan' : 'bg-hairline'}`} />
              <ArrowRight size={16} className={`shrink-0 transition-colors ${activeNode === 1 || activeNode === 2 ? 'text-vibe-cyan animate-pulse' : 'text-fg-muted'} `} />
            </div>
            <code className="text-[9px] font-mono text-fg-muted bg-overlay px-1.5 py-0.5 rounded border border-hairline mt-1">
              compute_bid(context)
            </code>
          </div>

          {/* Node 2: The Critic (Simulation Judge Agent) with Embedded Tool - Cols 8 to 12 */}
          <div 
            onClick={() => {
              setSelectedWiringCard('agent');
              setActiveTab('wiring');
            }}
            className={`lg:col-span-5 p-5 rounded-3xl border-2 transition-all space-y-3 relative cursor-pointer group ${
              activeNode === 3 || selectedWiringCard === 'agent'
          ...
          >
            ...
            <p className="text-xs text-fg-muted font-sans leading-relaxed">
              Audits candidate code against market microeconomics and dynamic budget pacing rules.
            </p>

            {/* Embedded Actuator Tool inside Critic */}
            <div 
              onClick={(e) => {
                e.stopPropagation();
              ...
              className={`p-3 rounded-2xl border transition-all cursor-pointer ${
                activeNode === 2 || selectedWiringCard === 'tool'
                  ? 'bg-blue-500/15 border-blue-500 text-blue-400 shadow-[0_0_20px_rgba(59,130,246,0.25)] ring-2 ring-blue-500/30'
                  : 'bg-overlay/60 border-hairline hover:border-blue-400/40'
              }`}
            >
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  <Terminal size={15} className="text-blue-400 shrink-0" />
                  <span className="text-[11px] font-mono font-bold text-fg truncate">
                    Critic's Tool: <code className="text-blue-400">evaluate_policy_code</code>
                  </span>
                </div>
                <span className="shrink-0 text-[9px] font-mono px-2 py-0.5 rounded-full bg-overlay border border-hairline text-fg-muted whitespace-nowrap">
                  Actuator Tool
                </span>
              </div>
              <p className="text-[10px] text-fg-muted mt-1 leading-snug">
                Critic runs candidate code against <strong className="text-fg">600,000 live auctions</strong> in-memory to extract spend, eCPM, and pacing survival.
              </p>
              <div className="text-[10px] font-mono py-1 px-2 rounded-xl bg-card border border-hairline flex items-center justify-between mt-2">
                <span className="text-fg-muted">Tool State:</span>
                <span className="font-bold text-fg">
                  {activeNode === 2 ? '📈 Simulating 600,000 Auctions...' : 'Ready for In-Memory Execution'}
                </span>
              </div>
            </div>
            ...
            <div className="text-[10px] font-mono py-1.5 px-3 rounded-xl bg-overlay border border-hairline flex items-center justify-between">
              ...
              <span className="font-bold text-fg">
                {activeNode === 3 ? '🔍 Formulating Score & Diagnostics...' : 'Score + Algorithmic Recommendations'}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="hidden lg:grid grid-cols-12 gap-4 -my-2 py-0.5 items-center relative z-20">
          {/* Left side: Upward feedback indicator returning to Node 1 */}
          <div className="col-span-6 flex items-center gap-2 pl-6">
            ...
            <span className={`text-[10px] font-mono ${activeNode === 5 ? 'text-amber-400 font-bold' : 'text-fg-muted'}`}>
              ▲ Mutated prompt context returns to Actor
            </span>
          </div>
          ...
          <div className="col-start-7 col-span-3 flex flex-col items-center">
            ...
            <div className={`w-0.5 h-3.5 ${activeNode >= 3 ? 'bg-purple-500' : 'bg-hairline'}`} />
            <ArrowDown size={13} className={activeNode >= 3 ? 'text-purple-400' : 'text-fg-muted'} />
          </div>
        </div>

        {/* LEVEL 2: DECISION ROUTING & CLOSED-LOOP REFINEMENT */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-stretch pt-2 relative">
          {/* 4. Feedback Loop (Proposer Node) - Cols 1 to 6 */}
          <div className={`lg:col-span-6 p-5 rounded-3xl border-2 transition-all flex flex-col justify-between space-y-3 relative overflow-hidden ${
              ...
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <h4 className="text-xs font-bold font-mono text-fg truncate">
                    Critic Diagnostics &amp; Prompt Mutation Loop
                  </h4>
                </div>
              </div>
            ...
            <div className="space-y-2">
              <p className="text-xs text-fg-muted font-sans leading-relaxed">
                When <code className="text-fg font-mono text-[10px]">score &lt; 99.5</code>, ADK's workflow routes through <code className="text-fg font-mono text-[10px]">proposer</code>, injecting the Critic's root-cause diagnostics into the Actor's prompt to refine the algorithm for the next round.
              </p>
            ...
            <div className="hidden lg:flex items-center gap-2 pt-1 text-[10px] font-mono font-bold">
              ...
              <span className={activeNode === 5 ? 'text-amber-400' : 'text-fg-muted'}>
                Cycles back into Node 1 (Actor Prompt Context for Round {currentRound < 3 ? currentRound + 1 : 3})
              </span>
            </div>
          </div>

          {/* 3. Router Node (Threshold Gate) - Cols 7 to 9 */}
          <div 
            onClick={() => setActiveTab('flywheel')}
            className={`lg:col-span-3 p-5 rounded-3xl border-2 transition-all space-y-3 flex flex-col justify-between cursor-pointer group ${
              activeNode === 4
          ...
          >
            <div className="space-y-3">
              ...
              <p className="text-xs font-mono text-fg-muted leading-tight">
          ...
          {/* 5. Production Ship (Champion Policy) - Cols 10 to 12 */}
          <div 
            onClick={() => setActiveTab('flywheel')}
            className={`lg:col-span-3 p-5 rounded-3xl border-2 transition-all space-y-3 flex flex-col justify-between cursor-pointer group ${
              activeNode === 6 || flywheelCompleted
          ...
          >
            <div className="space-y-3">
              ...
              <div>
                <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-emerald-400 block truncate">
                  5. Production Ship
                </span>
                <h4 className="text-xs font-bold font-mono text-fg truncate">
                  Champion Policy
              ...
              <p className="text-xs font-sans text-fg-muted leading-tight">
