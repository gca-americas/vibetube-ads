def compute_bid(context: dict) -> float:
    ...
    if daypart not in [
        "late_night",
        "morning",
    ]:  # For more competitive dayparts, react more to current P90
        base_bid = max(
            base_bid, p90 + 0.05
        )  # Ensure we are at least slightly above current P90
    ...
    if len(p90_history) >= 2:
        ...
        if velocity > 1.50:  # Escalating bidding war
            ...
            base_bid = (
                p90 + momentum_adjustment
            )  # Override base_bid to react immediately
    ...
    if hours_remaining > 0.5:  # Avoid division by zero or extreme pacing at very end
        ...
        if current_hourly_burn_rate > (
            target_hourly_burn * 1.2
        ):  # Behind pace, need to spend more
            pacing_factor = 1.08  # Scale bid up slightly (+8%)
        elif current_hourly_burn_rate < (
            target_hourly_burn * 0.8
        ):  # Ahead of pace / low budget, preserve budget
